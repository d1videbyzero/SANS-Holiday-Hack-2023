...
const pescadexPrevBtn = document.querySelector(
  ".cotd .pescadex button.prevBtn"
);
const pescadexNextBtn = document.querySelector(
  ".cotd .pescadex button.nextBtn"
);
const pescadexFINdexSeeWhatIDidThere = document.querySelector(
  ".cotd .pescadex span.findex"
);
...
const raceHighscore = document.querySelector(
  ".msgbox-raceresults .racehighscore"
);
const raceResultsClose = document.querySelector(
  ".msgbox-raceresults button.closeResults"
);
const viewScoresBtn = document.querySelector(
  ".msgbox-raceresults button.showScores"
);
...
settingsBtn.addEventListener("click", () => {
  ...
  document.querySelector('.sail-group[x-sail="1"] span').innerText = initialCap(
    me.config.colors[0]
  );
  document.querySelector('.sail-group[x-sail="2"] span').innerText = initialCap(
    me.config.colors[1]
  );
  document.querySelector('.sail-group[x-sail="3"] span').innerText = initialCap(
    me.config.colors[2]
  );
...
for (let sailIndex = 0; sailIndex < 3; sailIndex += 1) {
  document
    .querySelector(`.sail-group[x-sail="${sailIndex + 1}"] button.sailPrev`)
    .addEventListener("click", () => {
      const currentIndex = SailColors.indexOf(
        document
          .querySelector(`.sail-group[x-sail="${sailIndex + 1}"] span`)
          .innerText.toLowerCase()
      );
      let nextIndex = currentIndex - 1;
      if (nextIndex < 0) nextIndex = SailColors.length - 1;
      document.querySelector(
        `.sail-group[x-sail="${sailIndex + 1}"] span`
      ).innerText = initialCap(SailColors[nextIndex]);
    });
  document
    .querySelector(`.sail-group[x-sail="${sailIndex + 1}"] button.sailNext`)
    .addEventListener("click", () => {
      const currentIndex = SailColors.indexOf(
        document
          .querySelector(`.sail-group[x-sail="${sailIndex + 1}"] span`)
...
saveSettingsBtn.addEventListener("click", () => {
  ...
  const newSailColors = [
    document
      .querySelector('.sail-group[x-sail="1"] span')
      .innerText.toLowerCase(),
    document
      .querySelector('.sail-group[x-sail="2"] span')
      .innerText.toLowerCase(),
    document
      .querySelector('.sail-group[x-sail="3"] span')
      .innerText.toLowerCase(),
  ];
...
pescadexNextBtn.addEventListener("click", () => {
  ...
  pescadexIndex =
    pescadexIndex >= (playerData.fishCaught || []).length - 1
      ? (playerData.fishCaught || []).length - 1
      : pescadexIndex + 1;
  updatePescadex();
});
...
const updateUI = () => {
  ...
  openpescadexBtn.style.display =
    playerData.canFish && playerData.fishCaught.length ? "block" : "none";
  ...
  if (
    ...
    !me.fishing &&
    !me.race &&
    Math.abs(me.vx) < 0.001 &&
    Math.abs(me.vy) < 0.001
  ) {
    if (Math.abs(me.vx) < 0.001 && Math.abs(me.vy) < 0.001)
      castReelBtn.style.display = "block";
  } else {
  ...
  if (me.bearing) {
    setBearingBtn.innerHTML = `<i class="fa-solid fa-compass"></i> ${
      PORTS[me.bearing].name
    }`;
  } else {
...
const updatePescadex = () => {
  ...
  if ((playerData.fishCaught || []).length) {
    ...
    pescadexFINdexSeeWhatIDidThere.innerText = `${pescadexIndex + 1} of ${
      playerData.fishCaught.length
    }`;
...
const updatePorts = () => {
  ...
  const christmasPorts = Object.keys(PORTS).filter(
    (port) => PORTS[port].island === "Christmas Island"
  );
  const misfitPorts = Object.keys(PORTS).filter(
    (port) => PORTS[port].island === "Island of Misfit Toys"
  );
  const noirPorts = Object.keys(PORTS).filter(
    (port) => PORTS[port].island === "Film Noir Island"
  );
  const pixelPorts = Object.keys(PORTS).filter(
    (port) => PORTS[port].island === "Pixel Island"
  );
  const steampunkPorts = Object.keys(PORTS).filter(
    (port) => PORTS[port].island === "Steampunk Island"
  );
  const spacePorts = Object.keys(PORTS).filter(
    (port) => PORTS[port].island === "Space Island"
  );
  ...
  sections.forEach(([sectionName, portArray]) => {
    if (portArray.length) {
      ...
      portArray.forEach((portId) => {
        output += `<li x-data='${portId}' ${
          me.bearing === portId ? 'class="selected"' : ""
        }>${PORTS[portId].name}</li>`;
      });
      output += `</ul>`;
    }
  });
  ...
  portShortcuts.forEach((shortcut) => {
    ...
    shortcut.addEventListener("click", (event) => {
      if (event.target.attributes["x-data"].value === me.bearing) {
        ...
        Entities[playerData.uid].bearing =
          event.target.attributes["x-data"].value;
...
const websockHost =
  window.SEA_WS_HOST !== "UNSET"
    ? window.SEA_WS_HOST
    : window.SEA_WS_HOST === "UNSET"
    ? "ws://localhost:3000/sail"
    : "";
...
socket.addEventListener("message", (event) => {
  ...
  if (messageType === "e:") {
    ...
    const parsed = JSON.parse(payload);
    const idx = playerData.fishCaught.findIndex(
      (fish) => fish.name === parsed.fish.name
    );
    if (idx === -1) {
...
const handleKey = (event) => {
  ...
  if (
    ...
    if (event.type === "keydown") {
      ...
      if (event.key.indexOf("Arrow") === 0) {
        if (keyState & Keys[event.key.replace("Arrow", "").toUpperCase()])
          keyState ^= Keys[event.key.replace("Arrow", "").toUpperCase()];
      } else {
        if (keyState & Keys[event.key.toLowerCase()])
          keyState ^= Keys[event.key.toLowerCase()];
      }
    }
  }
...
const SailColors = ["white", "black", "green", "blue", "red", "pink", "yellow"];
...
var _0x19ba93 = _0x1c10;
(function (_0x4ecbb4, _0x258453) {
  var _0x8275fd = _0x1c10,
    _0xe1d82b = _0x4ecbb4();
  while (!![]) {
    try {
      var _0x4d9e1f =
        (-parseInt(_0x8275fd(0x11a)) / 0x1) *
          (parseInt(_0x8275fd(0x117)) / 0x2) +
        -parseInt(_0x8275fd(0x11b)) / 0x3 +
        parseInt(_0x8275fd(0x115)) / 0x4 +
        (parseInt(_0x8275fd(0x11d)) / 0x5) *
          (-parseInt(_0x8275fd(0x113)) / 0x6) +
        -parseInt(_0x8275fd(0x112)) / 0x7 +
        -parseInt(_0x8275fd(0x11c)) / 0x8 +
        parseInt(_0x8275fd(0x119)) / 0x9;
      if (_0x4d9e1f === _0x258453) break;
      else _0xe1d82b["push"](_0xe1d82b["shift"]());
    } catch (_0x5d04da) {
      _0xe1d82b["push"](_0xe1d82b["shift"]());
    }
  }
})(_0x44bf, 0xbdb46),
  (ImageAssets["lo_plaid"] = _0x19ba93(0x118)),
  (ImageAssets["mid_plaid"] = _0x19ba93(0x114)),
  (ImageAssets[_0x19ba93(0x111)] = _0x19ba93(0x116));
function _0x1c10(_0x5893b0, _0x249339) {
  var _0x44bf16 = _0x44bf();
  return (
    (_0x1c10 = function (_0x1c10ea, _0x2cfae9) {
      _0x1c10ea = _0x1c10ea - 0x111;
      var _0x17f156 = _0x44bf16[_0x1c10ea];
      return _0x17f156;
    }),
    _0x1c10(_0x5893b0, _0x249339)
  );
}
function _0x44bf() {
  var _0x3a8b5c = [
    ...
    "2852402gHadmq",
    "132wAwYbJ",
    "assets/SAIL/mid_plaid.png",
    "3807064BwgsIN",
    "assets/SAIL/top_plaid.png",
    "1382826GOnZle",
    "assets/SAIL/lo_plaid.png",
    "35969643WLNFVs",
    "1MaQQsJ",
  ];
  _0x44bf = function () {
...
const drawImageWithTransform = (
  ...
  ctx.drawImage(img, -origin.x, -origin.y, img.width, img.height);
  ctx.restore();
};
...
const drawMiniMap = () => {
  ...
  Object.keys(PORTS).forEach((portName) => {
    ctx.save();
    ctx.fillStyle =
      me.bearing === portName ? "rgba(0, 255, 255, 1)" : "rgba(255, 0, 0, 1)";
    ctx.beginPath();
  ...
  (HOTSPOTS || []).forEach((hotspot) => {
    drawImageWithTransform(
      ...
      0.2,
      {
        x: ImageAssets.startflag.width / 2,
        y: ImageAssets.startflag.height / 2,
      }
    );
  });
...
const drawCompass = () => {
  ...
  if (me.bearing) {
    ...
    drawImageWithTransform(
      ...
      1,
      1,
      { x: ImageAssets.arrow.width / 2, y: ImageAssets.arrow.height / 2 }
    );
  }
};
...
const renderShip = (
  ...
  for (let i = 1; i <= SQUIGGLES; i += 1) {
    ...
    ctx.bezierCurveTo(
      dest.x - SEGMENT_LENGTH * 0.66,
      dest.y - WAVE_AMPLITUDE,
      dest.x - SEGMENT_LENGTH * 0.33,
      dest.y + WAVE_AMPLITUDE,
      dest.x,
      dest.y
    );
  ...
  if (fishing) {
    drawImageWithTransform(
      onTheLine ? ImageAssets.roddown : ImageAssets.rod,
      x,
      y,
      angle,
      orientation,
  ...
  drawImageWithTransform(
    ImageAssets[`lo_${config.colors[0]}`],
    x,
    y,
    angle,
    orientation,
  ...
  drawImageWithTransform(
    ImageAssets[`mid_${config.colors[1]}`],
    x,
    y,
    angle,
    orientation,
  ...
  drawImageWithTransform(
    ImageAssets[`top_${config.colors[2]}`],
    x,
    y,
    angle,
    orientation,
...
const renderAhoy = (x, y, age) => {
  ...
  ctx.lineWidth = 1;
  ctx.fillText("AHOY!", point.x, point.y - 120 - (age - 2000) * 0.01);
  ctx.restore();
};
...
tweetBtn.addEventListener("click", () => {
  ...
  const postText = `I just caught a ${fish.name} in the Geese Islands while playing @SANSInstituteâ€™s Holiday Hack Challenge 2023! Come fish with me at https://sans.org/holidayhack. #HolidayHack`;
...
const renderScene = () => {
  ...
  if (me) {
    ...
    if (me.port) {
      ...
      portLabel.style.transform = `translate3d(${portPoint.x - 150}px, ${
        portPoint.y - 100
      }px, 0px)`;
    } else {
      if (portLabel.classList.contains("visible")) {
    ...
    drawImageWithTransform(
      ImageAssets.shadow,
      canvas.width / 2 + me.x * -WORLD_SCALE,
      canvas.height / 2 + me.y * -WORLD_SCALE,
      0,
      WORLD_SCALE,
      WORLD_SCALE
    );
    ...
    const layerDepths = terrainLayers.map((terrain, index) => [
      terrain[0],
      `t:${index}`,
    ]);
    const depthMap = [
      ...
      if (nextWaypoint) {
        ...
        if (me.raceIndex === me.race.waypoints.length - 1) {
          drawImageWithTransform(
            ...
            1,
            {
              x: ImageAssets.finishflag.width / 2,
              y: ImageAssets.finishflag.height / 2,
            }
          );
        } else {
          ctx.beginPath();
...
const handleCanvasCursor = (event) => {
  ...
  let relA =
    Math.atan2(relPosition.y, relPosition.x) * (180 / Math.PI) + 180 - 67.5;
